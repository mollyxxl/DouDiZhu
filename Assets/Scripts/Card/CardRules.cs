...
public class CardRules 
{
    ...
    {
        if (cards.Length < 4 || cards.Length % 4 != 0)
            return false;

        List<Weight> total=new List<Weight>();
        List<Card> singal = new List<Card>();
        List<Card> same = new List<Card>();
        //思路:拆分相同的在一起，单个的放到一起
        foreach (var card in cards)
        {
            total.Add(card.GetCardWeight());
        }

        foreach (var card in cards)
        {
            if (total.Contains(card.GetCardWeight()))
            {
                //存在相同的
                same.Add(card);
            }
        ...
        if (IsTripleStraight(same.ToArray()))
        {
            int rank = same.Count / 3;
            if (rank == singal.Count)
                return true;
        }
    ...
    public static bool IsTripleStraightAndDouble(Card[] cards)
    {
        ...
        //根据权重 分类统计次数
        Dictionary<Weight, int> dic = new Dictionary<Weight, int>();
        foreach (var card in cards)
        {
            var key=card.GetCardWeight();
    ...
    public  static bool IsJokerBoom(Card[] cards)
    {
        ...
        else if (cards[0].GetCardWeight() == Weight.SJoker)
        {
